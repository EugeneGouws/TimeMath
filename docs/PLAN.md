...
split in rebuilt files (GOVENDER, WYLIE); `LIB`/`ST`/`LAB` tokens map to null.
...
tale (CLAUDE.md § Known traps in the tooling).
...
**Do not warm-start from an existing timetable** — CLAUDE.md correction 8.
...
Regression fixtures are in CLAUDE.md. The Grade 12 2025 off-by-one flagged in
an earlier draft of this doc is **resolved** — CLAUDE.md's fixture table
reflects the live, post-exception workbook (98 students) and
`tests/test_roster.py::test_2025` asserts it directly.
